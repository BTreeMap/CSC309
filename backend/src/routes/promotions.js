 ...
 * Promotion Routes
 * 
 * Handles promotion CRUD operations.
 */
'use strict';
...
/**
 * Factory function to create the promotions router with dependencies
 */
function createPromotionsRouter(deps) {
    const { prisma, validateRequest, requireRole, hasRoleManager, isDefined } = deps;
        ...
        try {
            ...
            if (!isManager) {
                ...
                where.NOT = {
                    AND: [
                        { type: 'onetime' },
                        { usedBy: { some: { userId: req.auth.sub } } }
                    ]
                };
            } else {
            ...
            const [count, results] = await Promise.all([
                ...
                prisma.promotion.findMany({ where, skip, take: limitNum })
            ]);
            ...
            const formattedResults = results.map(promotion => {
                ...
                return isManager ? { ...base, startTime: promotion.startTime } : base;
            });
    ...
    router.patch('/:promotionId', requireRole('manager'), async (req, res) => {
        try {
            ...
            if (isDefined(name)) { updates.name = name; updatedFields.name = name; }
            if (isDefined(description)) { updates.description = description; updatedFields.description = description; }
            if (isDefined(type)) {
                const dbType = type === 'one-time' ? 'onetime' : type;
                updates.type = dbType;
            ...
            if (isDefined(startTime) || isDefined(endTime)) {
                ...
                if (startTime) { updates.startTime = start; updatedFields.startTime = start; }
                if (endTime) { updates.endTime = end; updatedFields.endTime = end; }
            }

            if (isDefined(minSpending)) {
                if (minSpending < 0) return res.status(400).json({ error: 'Invalid minSpending' });
                updates.minSpending = minSpending; updatedFields.minSpending = minSpending;
            }
            if (isDefined(rate)) {
                if (rate < 0) return res.status(400).json({ error: 'Invalid rate' });
                updates.rate = rate; updatedFields.rate = rate;
            }
            if (isDefined(points)) {
                if (points < 0) return res.status(400).json({ error: 'Invalid points' });
                updates.points = points; updatedFields.points = points;
            }

            const updated = await prisma.promotion.update({ where: { id: promotionId }, data: updates });
